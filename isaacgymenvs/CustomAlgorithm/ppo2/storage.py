...
class RolloutStorage:
    def __init__(self, num_envs, num_transitions_per_env, obs_shape, actions_shape, device, adv_norm=True, rnn=False, seq_len=1, rnn_states_shape=1):
        ...
        if self.rnn:
            ...
            self.rnn_states_shape[1]*=num_envs
            assert num_transitions_per_env % self.seq_len == 0  # 判断是否整除
        # Core
        # for buffer except rnn states, the buffer size is [length per ep, num env, shape]
        # for rnn states buffer, the buffer size is [number of seq per ep, state's layer size, num env, hidden units size]
        ...
        self.states = torch.zeros(
            [self.num_seq, *self.rnn_states_shape], dtype=torch.float32).to(self.device)
    ...
    def mini_batch_generator_inorder(self, num_mini_batches):
        ...
        mini_batch_size_mb = self.num_transitions_per_env//self.seq_len//num_mini_batches
